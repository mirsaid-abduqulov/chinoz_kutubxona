...
export class GenresService {
  ...
  async create(dto: CreateGenreDto) {
    ...
    if (existing) {
      throw new ConflictException('Genre with this name already exists in one of the languages');
    }
  ...
  async findAll(query: QueryGenreDto) {
    ...
    const [genres, total] = await Promise.all([
      this.prisma.genre.findMany({
        ...
        skip,
        take,
        // Since there is no created_at, sort by name_latin by default if sortBy is default
  ...
  async remove(id: string) {
    ...
    if (genre.books && genre.books.length > 0) {
      throw new ConflictException('Cannot delete genre because it is linked to books');
    }
